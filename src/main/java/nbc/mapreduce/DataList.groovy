...
class DataList {
    ...
    //**数据属性的种类数量，用于拉普拉斯平滑
    Map<String, Integer> propertiesCla = [:]
    //**数据集总数目
    long total = 0
    //**测试数据的分类
    String testCla
    ...
    void putTrains(MapWritable mapData) {
        ...
        //**每种分类的数量
        clas.containsKey(cla) ? clas.put(cla, clas.get(cla) + 1) : clas.put(cla, 1)
     ...
     ** *******************************************************
     */
    int getPropertiesClaCount(String cla) {
        this.propertiesCla.get(cla)
    }
